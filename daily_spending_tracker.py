...
class DaySpending():
    """This class handles the spending of a single
    day, and the basic type of the data should be in
    the following form:
    date, dict(category1, spending1, category2, spending2...)

    it also contains methods that can add or modify
    the data
    """
    ...
    def __str__(self) -> str:
        ...
        return f"On {make_ordinal(self.date)}, \
you spent ${self.category['food']} on food"\
...
class MonthSpending():
    ...
    def __init__(self, year: int, month: str,
        ...
        self.__items = (items if items is not None else
                        [None] * monthrange(year,
                                            MonthList.index(month) + 1)[1])

        if target >= 0:
            self.target = target
        else:
            raise ValueError("The target of this \
                month's spending must be positive number")
...
def load_month_from_file(filename: str) -> MonthSpending:
    ...
    with open(filename, mode='r') as csvfile:
        ...
        year, month, new_target = (int(first_line[0]),
                                   first_line[1], float(first_line[2]))
